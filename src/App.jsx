...
export default function App() {
  ...
  const rows = useMemo(() => {
    ...
    return list.filter((item) => {
      const categoryMatches = page === 'Categorias' || categoryFilter === 'Todas' || item.categoria_id === categoryFilter;
      const linkedPlace = page === 'Empresas' ? data.locais.find((place) => place.empresa_id === item.id) : null;
      const searchable = [item.nome_fantasia, item.nome, item.descricao, item.endereco, item.telefone, item.categoria?.nome, linkedPlace?.nome, linkedPlace?.descricao].filter(Boolean).join(' ').toLowerCase();
  ...
  return <div className="app-shell">
    <aside className={menuOpen ? 'sidebar open' : 'sidebar'}><div className="brand"><div className="brand-mark"><img src="/favicon.svg" alt="Agente Guia" style={{width: '100%', height: '100%'}} /></div><div><strong>Agente Guia</strong><small>Painel de gestão</small></div><button className="close" onClick={() => setMenuOpen(false)}>×</button></div><nav>{nav.map(([name, icon]) => <button className={page === name ? 'nav-item active' : 'nav-item'} key={name} onClick={() => { setPage(name); setMenuOpen(false); setSearch(''); setCategoryFilter('Todas'); }}><i>{icon}</i>{name}</button>)}</nav><div className="sidebar-bottom"><div className="profile"><div className="avatar">VG</div><div><b>{session.user.email?.split('@')[0] || 'Administrador'}</b><small>Administrador</small></div></div><button className="logout" onClick={() => supabase.auth.signOut()}>↪ Sair</button></div></aside>
    <main><header><button className="menu-button" onClick={() => setMenuOpen(true)}>☰</button><div className="crumb">Gestão <span>/</span> {page}</div><div className="header-actions"><button className="help">?</button><button className="avatar">VG</button></div></header><section className="content"><div className="title-row"><div><h1>{page === 'Empresas' ? 'Empresas parceiras' : title}</h1><p>{page === 'Empresas' ? `${data.empresas.length} no total` : subtitle}</p></div><button className="primary" onClick={() => { setEditingRecord(null); setModal(true); }}>＋ Nova {page.slice(0, -1)}</button></div>
      ...
      <section className={page === 'Empresas' ? 'partner-panel' : 'panel'}><div className="panel-top"><div className="search"><span>⌕</span><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={page === 'Locais' ? 'Buscar locais...' : `Buscar ${page.toLowerCase()}...`} /></div>{page !== 'Categorias' && <div className="category-filters"><button className={categoryFilter === 'Todas' ? 'category-filter active' : 'category-filter'} onClick={() => setCategoryFilter('Todas')}>Todas</button>{data.categorias.map((category) => <button className={categoryFilter === category.id ? 'category-filter active' : 'category-filter'} key={category.id} onClick={() => setCategoryFilter(category.id)}>{category.icone || '◇'} {category.nome}</button>)}</div>}</div><DataTable page={page} rows={rows} allPlaces={data.locais} loading={loading} onEdit={(record) => { setEditingRecord(record); setModal(true); }} onDelete={(record) => deleteRecord(record)} /></section>
    </section></main>
...
function DataTable({ page, rows, allPlaces, loading, onEdit, onDelete }) {
  ...
  if (company) return rows.length ? <div className="partner-grid">{rows.map((x) => { const place = allPlaces?.find((item) => item.empresa_id === x.id); return <article className="partner-card" key={x.id}><div className="partner-image">{place?.foto_capa_url ? <img src={place.foto_capa_url} alt={x.nome_fantasia} /> : <span>{x.nome_fantasia?.charAt(0) || 'A'}</span>}</div><div className="partner-body"><h3>{x.nome_fantasia}</h3><p className="partner-meta">{x.categoria?.icone || '◇'} {x.categoria?.nome || 'Sem categoria'}{place?.endereco ? ` · ${place.endereco}` : ''}</p><p className="partner-description">{place?.descricao || 'Empresa parceira do Agente Guia.'}</p><div className="partner-footer"><small>{place?.fotos_extra?.length || place?.fotos?.length || 0} foto(s) extra</small><button type="button" className="edit-button" onClick={() => onEdit(x)}>✎ Editar</button></div></div></article>; })}</div> : <div className="empty">{loading ? 'Carregando dados...' : 'Nenhum resultado encontrado.'}</div>;
...
function Editor({ page, categories, companies, record, close, save, loading }) {
  ...
  return <><div className="modal-layer"><form className="modal" onSubmit={submit}><div className="modal-header"><div><h2>{record ? 'Editar' : 'Nova'} {page.slice(0, -1)}</h2><p>{record ? 'Atualize os dados abaixo.' : 'Preencha os dados abaixo para cadastrar.'}</p></div><button type="button" onClick={close}>×</button></div><div className="form-grid">
    ...
    {isPlace && <><Field label="Nome"><Input name="nome" defaultValue={record?.nome || ''} required placeholder="Nome do local" /></Field><Field label="Categoria"><select name="categoria_id" required defaultValue={record?.categoria_id || ''}><option value="" disabled>Selecione uma categoria</option>{categories.map(x => <option value={x.id} key={x.id}>{x.nome}</option>)}</select></Field><Field label="Empresa vinculada"><select name="empresa_id" defaultValue={record?.empresa_id || ''}><option value="">Ponto público (sem empresa)</option>{companies.map(x => <option value={x.id} key={x.id}>{x.nome_fantasia}</option>)}</select></Field><Field label="Faixa de preço"><select name="faixa_preco" defaultValue={record?.faixa_preco || ''}><option value="">Não informado</option><option value="$">$</option><option value="$$">$$</option><option value="$$$">$$$</option></select></Field><Field label="Descrição" full><textarea name="descricao" defaultValue={record?.descricao || ''} placeholder="O que é este lugar?" /></Field><Field label="História e curiosidades" full><textarea name="historia" defaultValue={record?.historia || ''} placeholder="Informações que o agente contará ao recomendar" /></Field><Field label="Endereço" full><Input name="endereco" defaultValue={record?.endereco || ''} placeholder="Rua, número, bairro" /></Field><Field label="Telefone"><Input name="telefone" defaultValue={record?.telefone || ''} placeholder="(00) 00000-0000" /></Field><Field label="Instagram"><Input name="instagram" defaultValue={record?.instagram || ''} placeholder="@perfil" /></Field><Field label="Horário de funcionamento" full><Input name="horario_funcionamento" defaultValue={record?.horario_funcionamento || ''} placeholder="Segunda a sábado, 9h às 18h" /></Field><Field label="Link Google Maps" full><Input name="link_google_maps" type="url" onChange={handleInputChange} defaultValue={record?.link_google_maps || ''} placeholder="https://maps.google.com/?q=..." /></Field><div style={{display: 'grid', gridTemplateColumns: '1fr 120px', gap: '12px', alignItems: 'start'}}><Field label="URL da Foto de Capa" full><Input name="foto_capa_url" type="url" onChange={handleInputChange} defaultValue={record?.foto_capa_url || ''} placeholder="https://exemplo.com/foto.jpg" /></Field>{previewUrl && <div style={{textAlign: 'center'}}><img src={previewUrl} alt="Preview" onClick={() => setExpandedImage(previewUrl)} style={{maxWidth: '120px', maxHeight: '120px', borderRadius: '6px', border: '1px solid #dce4df', cursor: 'pointer', transition: 'transform 0.2s'}} onMouseEnter={(e) => e.target.style.transform = 'scale(1.05)'} onMouseLeave={(e) => e.target.style.transform = 'scale(1)'} onError={() => setPreviewUrl('')} /></div>}</div><Field label="Link Google Maps Curto" full><Input name="link_google_maps_curto" type="url" onChange={handleInputChange} defaultValue={record?.link_google_maps_curto || ''} placeholder="https://goo.gl/..." /></Field><Field label="Latitude"><Input name="latitude" type="number" step="any" defaultValue={record?.latitude ?? extractLatLng(record?.link_google_maps || '').lat} placeholder="-3.71722" /></Field><Field label="Longitude"><Input name="longitude" type="number" step="any" defaultValue={record?.longitude ?? extractLatLng(record?.link_google_maps || '').lng} placeholder="-38.54306" /></Field><Field full label=""><span className="check-line"><input name="ativo" type="checkbox" defaultChecked={record?.ativo ?? true} /> Local ativo e visível no Guia</span></Field></>}
  ...
  </div><div className="modal-actions"><button type="button" className="secondary" onClick={close}>Cancelar</button><button className="primary" disabled={loading}>{loading ? 'Salvando...' : `Salvar ${page.slice(0, -1)}`}</button></div></form></div>{expandedImage && <div style={{position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000}} onClick={() => setExpandedImage(null)}><div style={{position: 'relative', background: '#fff', padding: '20px', borderRadius: '12px', maxWidth: '90vw', maxHeight: '90vh'}}><button style={{position: 'absolute', top: '10px', right: '10px', background: 'none', border: 'none', fontSize: '24px', cursor: 'pointer', color: '#666'}} onClick={() => setExpandedImage(null)}>×</button><img src={expandedImage} alt="Visualização Expandida" style={{maxWidth: '100%', maxHeight: 'calc(90vh - 60px)', objectFit: 'contain'}} onClick={(e) => e.stopPropagation()} /></div></div>}</>
}
